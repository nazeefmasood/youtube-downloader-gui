...
export class Downloader extends EventEmitter {
  ...
  async download(options: DownloadOptions): Promise<void> {
    ...
    // Subtitle options
    if (subtitleOptions?.enabled || downloadAllSubtitles) {
      args.push('--write-subs')
      if (subtitleOptions?.includeAutoGenerated || downloadAllSubtitles) {
        args.push('--write-auto-subs')
      }

      // If downloadAllSubtitles is enabled, download all available languages
      if (downloadAllSubtitles) {
        args.push('--sub-langs', 'all')
        logger.info('Downloading all available subtitles')
      } else if (subtitleOptions?.languages && subtitleOptions.languages.length > 0) {
        // Use specified languages
        const langs = subtitleOptions.languages.join(',')
