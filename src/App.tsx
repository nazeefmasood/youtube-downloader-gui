...
function App() {
  ...
  const getVideoStatus = (index: number) => {
    ...
    if (downloadProgress && index < (downloadProgress.currentIndex || 0))
      return "completed";
    return "waiting";
  };
  ...
  const currentQueueItem = queueStatus.items.find(
    (i) => i.id === queueStatus.currentItemId,
  );
  ...
  return (
    <>
              ...
              {!contentInfo && !isDetecting && !hasQueueItems && (
                <div className="empty-state">
                  ...
                  <div className="empty-state-text">
                    // Paste a YouTube URL above to begin analysis
                  </div>
              ...
              {isDetecting && !contentInfo && (
                <div className="content-loading">
                  ...
                  <div className="loading-title">
                    ANALYZING TARGET<span className="blink">_</span>
                  </div>
                  ...
                  <div className="content-info">
                    <div className="content-header">
                      ...
                      <div className="content-details">
                        <span className={`content-type ${contentInfo.type}`}>
                          {contentInfo.type}
                        </span>
                        ...
                        <div className="content-meta">
                          ...
                          {contentInfo.videoCount &&
                            contentInfo.videoCount > 1 && (
                              <span className="content-meta-item">
                                {contentInfo.videoCount} VIDEOS
                              </span>
                            )}
                          ...
                          {contentInfo.entries.map((video) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {isDownloading && (
                                  <span
                                    className={`video-list-status ${status}`}
                                  >
                  ...
                  <div className="queue-header">
                    <div className="queue-stats">
                      ...
                      <div className="queue-badges">
                        ...
                        {queueStatus.items.filter(
                          (i) =>
                            i.status === "pending" || i.status === "paused",
                        ).length > 0 && (
                        ...
                        <div className="queue-item-info">
                          ...
                          <div className="queue-item-meta">
                            <span className="queue-item-source">
                              {item.source === "extension" ? "EXT" : "APP"}
                            </span>
                            <span className="queue-item-format">
                              {item.audioOnly ? "AUDIO" : "VIDEO"}
                            </span>
              ...
              <div className="side-panel">
                {
                  <>
                    ...
                    <div className="panel-actions">
                      {isDownloading ? (
                        <button
                          className="btn-download btn-stop"
      ...
      {isActiveDownload && view === "downloads" && (
        <div
          className={`progress-bar-fixed ${contentInfo && !showSuccess ? "with-sidebar" : ""}`}
        >
          <div className="progress-bar-inner">
            <div className="progress-info">
              ...
              <span className="progress-percent">
                {effectiveProgress?.percent?.toFixed(0) || 0}%
              </span>
            </div>
            <div className="progress-bar">
              <div className="progress-bar-bg" />
              <div
                className={`progress-fill ${!effectiveProgress ? "progress-fill-init" : ""}`}
                style={{ width: `${effectiveProgress?.percent || 0}%` }}
              />
            </div>
            <div className="progress-stats-compact">
              <span>
                <strong>{effectiveProgress?.speed || "--"}</strong> Speed
              </span>
              <span>
                <strong>{effectiveProgress?.eta || "--"}</strong> ETA
              </span>
              <span>
                <strong>{effectiveProgress?.total || "--"}</strong> Size
              </span>
              {effectiveProgress?.totalFiles &&
                effectiveProgress.totalFiles > 1 && (
                  <span>
                    Video{" "}
                    <strong>
                      {effectiveProgress.currentIndex || 1}/
                      {effectiveProgress.totalFiles}
                    </strong>
                  </span>
                )}
      ...
      <div className="status-bar">
        <div className="status-bar-left">
          <div
            className={`status-indicator ${isActiveDownload ? "downloading" : ""} ${downloadError ? "error" : ""} ${queueStatus.isPaused ? "paused" : ""} ${queueJustFinished ? "complete" : ""}`}
          />
          <span>
            {isActiveDownload &&
              effectiveProgress?.status === "downloading" &&
              `DOWNLOADING ${effectiveProgress.percent?.toFixed(0)}%`}
            {isActiveDownload &&
              effectiveProgress?.status === "merging" &&
              "MERGING FILES"}
            {isActiveDownload &&
              effectiveProgress?.status === "waiting" &&
              "WAITING FOR NEXT"}
